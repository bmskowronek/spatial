...
def build_graph(node, graph=None, parent=None, depth=0):
    if graph is None:
        graph = nx.Graph()
    
    if node is not None:
        graph.add_node(node.index, pos=(node.x, node.y), label=f'{node.index}\n({node.x}, {node.y})')
        if parent is not None:
            graph.add_edge(parent.index, node.index)
        
        build_graph(node.left, graph, node, depth + 1)
        build_graph(node.right, graph, node, depth + 1)
    
    return graph

# Assuming `root` is the root of your KD-tree
graph = build_graph(kd_tree.root)


def visualize_kdtree(graph, depth_limit=15):
    net = Network(notebook=True, height="750px", width="100%")
    subgraph = nx.ego_graph(graph, 0, radius=depth_limit)  # Adjust '0' if your root node has a different identifier

    for node, node_attrs in subgraph.nodes(data=True):
        net.add_node(node, label=node_attrs['label'], title=f"Index: {node}, Position: {node_attrs['pos']}", value=10)

    for node1, node2 in subgraph.edges():
        net.add_edge(node1, node2)

    net.show("kdtree.html")


visualize_kdtree(graph)
